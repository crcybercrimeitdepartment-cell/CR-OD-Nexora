...
const dir = path.join(__dirname, 'src', 'page', 'PlatformSettings');
const files = fs.readdirSync(dir);

const excluded = ['CustomiseThemePage.jsx'];

files.forEach(file => {
  if (file.endsWith('.jsx') && !excluded.includes(file)) {
    const filePath = path.join(dir, file);
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Extract title and description from dummyData
    let titleMatch = content.match(/title:\s*"([^"]+)"/);
    let descMatch = content.match(/description:\s*"([^"]+)"/);
    
    let title = titleMatch ? titleMatch[1] : file.replace('.jsx', '');
    let desc = descMatch ? descMatch[1] : 'Module under development.';
    
    const newContent = `import React from 'react';

export default function ${file.replace('.jsx', '')}({ onBack }) {
  return (
    <div className="w-full flex-1 flex flex-col items-center justify-center min-h-[50vh] p-8">
      ...
      <h2 className="text-2xl sm:text-3xl font-black text-[#1e2a52] mb-4 text-center tracking-tight">${title}</h2>
