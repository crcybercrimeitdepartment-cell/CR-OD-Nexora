import { GHLRI_TOOLS } from '../data/subTools';
...
import RILPage from "./GHLRI-GuestHouse&LodgeRecordIntelligence/RIL-RestaurantInformationList";
...
export default function GHLRIPage({ onBack }) {
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      if (event.state && event.state.page === 'GHLRI' && event.state.subPage) {
  ...
  const handleSelectSubPage = (id) => {
    window.history.pushState({ page: 'GHLRI', subPage: id }, '', '#GHLRI-' + id);
  ...
  if (selectedSubPage === 'ghi') return <GHIPage onBack={handleSubPageBack} />;
  if (selectedSubPage === 'ghlri') return <GHLRI_SubPage onBack={handleSubPageBack} />;
  if (selectedSubPage === 'bil') return <BILPage onBack={handleSubPageBack} />;
  if (selectedSubPage === 'ril') return <RILPage onBack={handleSubPageBack} />;
