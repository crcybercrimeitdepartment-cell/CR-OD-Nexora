/**
 * @file src/page/LegalPdfIntelligence/DocumentManagement/PrintBooklet/PrintBookletPage.jsx
 * @description Print Booklet tool page component.
 */
...
export default function PrintBookletPage({ onBack }) {
  return (
    <section aria-labelledby="printbooklet-title" className="flex flex-col gap-6 sm:gap-7">
      <header className="relative pb-2 sm:pb-3">
        ...
        <div className="mt-7 text-center md:mt-0 md:px-28">
          <h1 id="printbooklet-title" className="text-3xl font-black leading-tight tracking-tight text-[#1e2a52] sm:text-4xl lg:text-5xl">
            Print Booklet
          </h1>
          <p className="mx-auto mt-4 max-w-3xl text-sm font-semibold leading-relaxed text-slate-700 sm:text-base">
            Print pages in double-sided booklet, brochure, and magazine folding layouts.
          </p>
        </div>
      </header>
      ...
      <div className="mx-auto w-full max-w-4xl rounded-3xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8">
        <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-2xl p-10 text-center bg-slate-50/50 hover:bg-slate-50 transition">
          ...
          <p className="text-sm text-slate-600 mb-6 max-w-md">
            Catalog, index, tag, and organize files for Print Booklet.
          </p>
