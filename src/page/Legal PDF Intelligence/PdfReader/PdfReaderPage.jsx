          ...
          {filteredTools.map((tool) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center p-3 gap-2 rounded-xl border transition-all duration-200 group ${!isReady ? 'opacity-50 cursor-not-allowed border-slate-100 bg-slate-50' :
                    isActive
              ...
              >
                ...
                <span className={`text-[10px] font-bold text-center leading-tight ${isActive ? 'text-blue-700' : 'text-slate-600'
                  }`}>
                  {tool.name}
                </span>
              </button>
...
export function PDFMainView({
  ...
  return (
    <div
      ...
      className={`absolute inset-0 w-full h-full ${(isPresenting || isMouseNavigationEnabled) ? 'overflow-hidden' : 'overflow-auto'} flex justify-center py-6 px-2 transition-colors duration-300 ${isNightMode ? 'bg-slate-900' : 'bg-slate-100/50'
        }`}
      style={{
            ...
            {sidebarMode === 'bookmarks' ? (
              ...
              Array.from(new Array(numPages), (el, index) => (
                <div
                  ...
                  className={`group flex lg:flex-col items-center gap-1.5 cursor-pointer transition-all shrink-0 ${currentPage === index + 1 ? 'scale-100' : 'hover:scale-[1.02]'
                    }`}
                >
                  <div className={`w-16 h-20 lg:w-[68px] lg:h-24 rounded shadow-sm border-2 flex items-center justify-center bg-slate-50 transition-colors relative overflow-hidden shrink-0 ${currentPage === index + 1 ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200 group-hover:border-blue-300'
                    }`}>
                    <Page
                      pageNumber={index + 1}
                      width={64}
                  ...
                  <span className={`text-xs font-bold lg:mt-1 ${currentPage === index + 1 ? 'text-blue-600' : 'text-slate-500'
                    }`}>
                    {index + 1}
                  </span>
                </div>
...
export function PDFUploadArea({ onUpload }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-4 mb-8 animate-fade-in-up">
      <div
        className={`relative w-full rounded-2xl border-2 border-dashed p-10 flex flex-col items-center justify-center transition-all duration-300 ease-out bg-white shadow-sm ${isDragging ? 'border-blue-500 bg-blue-50/50 scale-[1.02]' : 'border-slate-300 hover:border-blue-400 hover:bg-slate-50'
          }`}
      ...
      {!props.isPresenting && (
        <div className="w-full bg-white rounded-xl border border-slate-200 shadow-sm p-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-xs border ${pdfFile ? 'bg-red-50 text-red-500 border-red-100' : 'bg-slate-50 text-slate-400 border-slate-200'
              }`}>
              PDF
            </div>
            <div className="flex flex-col">
