...
export default function CopyrightRenewalReminderPage({ tool, onBack }) {
  ...
  const loadStatus = async () => {
    ...
    try {
      if (presets.length === 0) {
        try {
          const presetsR = await fetch('/api/pdf-copyright-protection/renewal/presets');
      ...
      const r = await fetch('/api/pdf-copyright-protection/renewal/status', { method: 'POST', body: fd });
  ...
  const saveReminder = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/renewal/configure', { method: 'POST', body: fd });
      const d = await r.json();
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/renewal/report', { method: 'POST', body: fd });
      const d = await r.json();
