...
export default function LicenseManagementPage({ tool, onBack }) {
  ...
  const loadPresetsAndRead = async (selectedFile) => {
    ...
    try {
      const pr = await fetch('/api/pdf-copyright-protection/license/presets', { method: 'GET' });
      const pj = await pr.json();
      if (pj.success) setPresets(pj.presets || {});
    } catch (_) {}
    try {
      const fd = new FormData(); fd.append('file', selectedFile);
      const r = await fetch('/api/pdf-copyright-protection/license/read', { method: 'POST', body: fd });
  ...
  const saveLicense = async (e) => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/license/save', { method: 'POST', body: fd });
      const j = await r.json();
      if (j.success) {
        let verified = 'No';
        try { const vr = await fetch('/api/pdf-copyright-protection/license/verify/' + j.session_id); const vj = await vr.json(); verified = vj.verified ? 'Yes' : 'No'; } catch (_) {}
        ...
        {result && (
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 sm:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <a href={result.downloadUrl} download className="bg-amber-600 hover:bg-amber-700 text-white px-8 py-3.5 rounded-full font-bold shadow-md transition-all text-sm cursor-pointer inline-flex justify-center items-center gap-2 hover:scale-105 active:scale-95 w-full sm:w-auto">
