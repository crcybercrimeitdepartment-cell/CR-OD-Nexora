...
export default function CopyrightHolderManagementPage({ tool, onBack }) {
  ...
  const readHolders = async (f) => {
    ...
    try {
      const r = await fetch('/api/pdf-copyright-protection/holder-management/read', { method: 'POST', body: fd });
  ...
  const saveToPdf = async () => {
    ...
    try {
      const r = await fetch('/api/pdf-copyright-protection/holder-management/save', { method: 'POST', body: fd });
      const j = await r.json();
      
      if (j.success) {
        let verified = 'Unknown';
        try {
          const verifyR = await fetch('/api/pdf-copyright-protection/holder-management/verify/' + j.session_id, { method: 'GET' });
          if (verifyR.ok) {
