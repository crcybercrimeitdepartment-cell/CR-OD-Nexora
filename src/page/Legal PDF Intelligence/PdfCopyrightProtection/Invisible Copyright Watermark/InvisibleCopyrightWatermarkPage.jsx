...
export default function InvisibleCopyrightWatermarkPage({ tool, onBack }) {
  ...
  const embed = async (e) => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/invisible-watermark/embed', { method: 'POST', body: fd });
  ...
  const verifyWatermark = async (file) => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/invisible-watermark/verify', { method: 'POST', body: fd });
      const j = await r.json();
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <form onSubmit={embed} className="space-y-4 bg-violet-50/30 p-5 rounded-2xl border border-violet-100/50">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">Date</label>
                    <div className="relative">
                      <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" />
                      <input type="date" name="year" value={formData.year} onChange={handleFormChange}
                        className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-white text-slate-800 font-medium text-sm" />
                    </div>
                  </div>
                </div>
          ...
          {embedResult && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <a href={embedResult.downloadUrl} download
