...
export default function CopyrightPolicyTemplatesPage({ tool, onBack }) {
  ...
  const loadAndRead = async (selectedFile) => {
    ...
    let loadedTemplates = {};
    try {
      const pr = await fetch('/api/pdf-copyright-protection/policy/templates', { method: 'GET' });
    ...
    try {
      const r = await fetch('/api/pdf-copyright-protection/policy/read', { method: 'POST', body: fd });
  ...
  const applyPolicy = async (e) => {
    ...
    try {
      const r = await fetch('/api/pdf-copyright-protection/policy/apply', { method: 'POST', body: fd });
      const j = await r.json();
      
      if (j.success) {
        let verified = false;
        try {
          const vr = await fetch('/api/pdf-copyright-protection/policy/verify/' + j.session_id, { method: 'GET' });
