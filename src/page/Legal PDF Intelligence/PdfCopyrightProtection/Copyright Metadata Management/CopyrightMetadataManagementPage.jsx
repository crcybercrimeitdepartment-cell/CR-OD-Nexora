...
export default function CopyrightMetadataManagementPage({ tool, onBack }) {
  ...
  const readMetadata = async (selectedFile) => {
    ...
    try {
      const res = await fetch('/api/pdf-copyright-protection/metadata/read', { method: 'POST', body: fd });
  ...
  const submitUpdate = async (e) => {
    ...
    try {
      const res = await fetch('/api/pdf-copyright-protection/metadata/update', { method: 'POST', body: fd });
      const data = await res.json();
