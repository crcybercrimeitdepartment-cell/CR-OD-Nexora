...
export default function PublisherInformationPage({ tool, onBack }) {
  ...
  const readPublisher = async (selectedFile) => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/publisher/read', { method: 'POST', body: fd });
  ...
  const submitUpdate = async (e) => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/publisher/update', { method: 'POST', body: fd });
      const j = await r.json();
      if (j.success) {
        ...
        {result && (
          <div className="bg-white rounded-3xl border border-slate-200/80 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 sm:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4">
              <a href={result.downloadUrl} download className="bg-teal-600 hover:bg-teal-700 text-white px-8 py-3.5 rounded-full font-bold shadow-md transition-all text-sm cursor-pointer inline-flex justify-center items-center gap-2 hover:scale-105 active:scale-95 w-full sm:w-auto">
