...
export default function DocumentProvenanceTrackingPage({ tool, onBack }) {
  ...
  const track = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/pdf-copyright-protection/provenance/track', { method: 'POST', body: fd });
  ...
  const generateReport = async () => {
    ...
    try {
      const fd = new FormData(); fd.append('file', files[0].originalFile);
      const r = await fetch('/api/pdf-copyright-protection/provenance/report', { method: 'POST', body: fd });
      const d = await r.json();
