...
export default function AuthorVerificationPage({ tool, onBack }) {
  ...
  const readMetadata = async (selectedFile) => {
    ...
    try {
      const r = await fetch('/api/pdf-copyright-protection/author-verification/extract', { method: 'POST', body: fd });
  ...
  const verify = async (e) => {
    ...
    try {
      const r = await fetch('/api/pdf-copyright-protection/author-verification/verify', { method: 'POST', body: fd });
      const j = await r.json();
