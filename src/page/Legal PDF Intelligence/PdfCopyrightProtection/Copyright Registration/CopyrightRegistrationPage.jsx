...
export default function CopyrightRegistrationPage({ tool, onBack }) {
  ...
  const checkExistingRegistration = async (selectedFile) => {
    ...
    try {
      const res = await fetch('/api/pdf-copyright-protection/registration/check', { method: 'POST', body: fd });
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch('/api/pdf-copyright-protection/registration', { method: 'POST', body: fd });
      const data = await res.json();
