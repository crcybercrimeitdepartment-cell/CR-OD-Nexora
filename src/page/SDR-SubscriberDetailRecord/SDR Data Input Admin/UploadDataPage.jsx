...
export default function SDRDataStoragePage({ onBack }) {
  React.useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
    ...
    <div className="flex-1 flex flex-col w-full relative pt-11 sm:pt-4">
        ...
        <div className="flex items-center justify-center w-full relative z-20">
          <div className="flex-1 text-center flex flex-col items-center justify-center min-w-0 pt-1 sm:pt-2 md:pt-3 px-2">
            <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-[#1e2a52] tracking-tight leading-tight break-words pb-1">
              <span>SDR Data Storage</span>
            </h1>
            <p className="mt-3 sm:mt-4 text-xs sm:text-sm md:text-base font-semibold text-slate-700 max-w-2xl mx-auto leading-relaxed">
              Monitor storage usage and database archives.
            </p>
          </div>
        </div>
      </header>
      <div className="flex-1 flex flex-col w-full max-w-[1720px] mx-auto px-4 sm:px-6 md:px-10 py-2">
        <div className="max-w-3xl mx-auto w-full bg-white rounded-2xl border border-slate-200/90 shadow-sm p-8 text-center text-slate-500">
           SDR Data Storage functionality coming soon.
        </div>
      </div>
    </div>
  );
