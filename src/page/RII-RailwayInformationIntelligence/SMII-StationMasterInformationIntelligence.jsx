...
export function Header({ title, description }) {
  const displayTitle = title || "Station Master Information Intelligence";
  const displayDesc = description || "Review station master logs, approvals & incident reports. Gain insights into daily railway operations, emergency interventions, and administrative decisions at the station level. Correlate official logs with passenger data to investigate delays, accidents, and security breaches with complete operational context.";
 ...
 */
export default function SMIIPage({ onBack }) {
  React.useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
      ...
      <div className="flex-1 flex flex-col w-full max-w-[1720px] mx-auto px-4 sm:px-6 md:px-10 py-4 overflow-x-hidden">
        <main className="flex-1 pt-1 pb-4">
          <div className="flex flex-col items-center justify-center py-12 sm:py-20 px-4 opacity-0 animate-fade-in" style={{ animation: 'fadeIn 0.5s ease-out forwards', animationDelay: '0.2s' }}>
            ...
            <p className="text-xs sm:text-sm md:text-base text-slate-500 font-medium text-center max-w-lg leading-relaxed mt-2">
              We are actively developing powerful new analytics tools for this module. These features will be available in the next major update.
            </p>
            <style jsx>{`
              @keyframes fadeIn {
