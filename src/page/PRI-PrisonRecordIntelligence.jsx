import { PRI_TOOLS } from '../data/subTools';
import React, { useState, useEffect } from 'react';
import ToolCard from '../components/nexora';
import { usePageLayout } from '../components/usePageLayout';

import PHIPage from "./PRI-PrisonRecordIntelligence/PHI-PrisonerHistoryIntelligence";
import JHIPage from "./PRI-PrisonRecordIntelligence/JHI-JailHistoryIntelligence";
import DEMO1Page from "./PRI-PrisonRecordIntelligence/PrisonDEMO1";
import DEMO2Page from "./PRI-PrisonRecordIntelligence/PrisonDEMO2";
 ...
 */
export default function PRIPage({ onBack }) {
  const { dynamicGridClass, displayTools } = usePageLayout('pri', PRI_TOOLS);
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      if (event.state && event.state.page === 'PRI' && event.state.subPage) {
  ...
  const handleSelectSubPage = (id) => {
    window.history.pushState({ page: 'PRI', subPage: id }, '', '#PRI-' + id);
  ...
  if (selectedSubPage === 'phi') return <PHIPage onBack={handleSubPageBack} />;
  if (selectedSubPage === 'jhi') return <JHIPage onBack={handleSubPageBack} />;
  if (selectedSubPage === 'demo1') return <DEMO1Page onBack={handleSubPageBack} />;
  if (selectedSubPage === 'demo2') return <DEMO2Page onBack={handleSubPageBack} />;
