...
export default function NexoraAgent({ onNavigateToModule, searchQuery, onSearchChange }) {
  ...
  const [activeTab, setActiveTab] = useState(null); // 'overview' | 'purpose' | 'work' | 'benefits' | 'useCases' | 'featureMenu'
  ...
  const handleSelectCard = (cardKey, autoTab = null, userQueryText = null) => {
    ...
    if (autoTab && ['overview', 'purpose', 'work', 'benefits', 'useCases', 'featureMenu'].includes(autoTab)) {
      setChatHistory(prev => [
        ...prev,
        {
          sender: 'user',
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="relative z-10 bg-white/40 backdrop-blur-xl border-b border-white/60 p-2.5 shrink-0 overflow-x-auto overscroll-contain scrollbar-none flex items-center gap-1.5 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
            ...
            {/* CHAT MESSAGES BODY */}
            <div ref={chatContainerRef} className="relative z-10 flex-1 min-h-0 overflow-y-auto overscroll-contain p-4 space-y-4 bg-[#f4f8ff]/50">
                    ...
                    <div className="w-full space-y-3">
                      ...
                      {msg.type === 'greeting' && (
                        <div className="relative overflow-hidden bg-gradient-to-br from-white/65 via-white/40 to-blue-100/30 backdrop-blur-2xl border border-white/80 rounded-2xl p-4 shadow-[0_12px_40px_rgba(30,42,82,0.12),inset_0_1px_1px_rgba(255,255,255,0.8)] text-xs font-medium text-slate-800 space-y-3.5">
                          ...
                          <div className={`relative z-10 grid grid-cols-2 gap-2.5 pt-1 ${showAllGreetingCards ? 'max-h-[260px] overflow-y-auto overscroll-contain pr-1 scrollbar-thin' : ''}`}>
                      ...
                      {msg.type === 'card_prompt' && (
                        <div className="relative overflow-hidden bg-gradient-to-br from-white/65 via-white/40 to-blue-100/30 backdrop-blur-2xl border border-white/80 rounded-2xl p-4 shadow-[0_12px_40px_rgba(30,42,82,0.12),inset_0_1px_1px_rgba(255,255,255,0.8)] space-y-3.5 border-l-4 border-l-blue-600">
                          ...
                          {/* 6 OPTION BUTTONS (HYPER-TRANSPARENT GLASS TILES) */}
                          <div className="relative z-10 grid grid-cols-2 sm:grid-cols-3 gap-2.5 pt-1">
                            {[
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-center gap-2 p-2.5 bg-gradient-to-br from-white/50 to-white/20 hover:from-[#1e2a52] hover:to-[#2b3a6d] hover:text-white backdrop-blur-2xl border border-white/80 ${opt.hoverGlow} rounded-xl transition-all duration-300 text-left font-bold text-xs text-slate-900 group shadow-[0_4px_16px_rgba(31,38,135,0.07),inset_0_1px_1px_rgba(255,255,255,0.8)] hover:shadow-lg hover:-translate-y-1`}
                                >
                                  <div className="w-6.5 h-6.5 rounded-lg bg-white/50 group-hover:bg-white/20 backdrop-blur-md flex items-center justify-center shrink-0 border border-white/70 transition-colors">
                                    <Icon className={`w-3.5 h-3.5 ${opt.color} group-hover:text-white`} />
                                  </div>
                      ...
                      {msg.type === 'tab_response' && (
                        <div className="relative overflow-hidden bg-gradient-to-br from-white/65 via-white/40 to-indigo-100/30 backdrop-blur-2xl border border-white/80 rounded-2xl p-4 shadow-[0_12px_40px_rgba(30,42,82,0.12),inset_0_1px_1px_rgba(255,255,255,0.8)] space-y-3.5 border-l-4 border-l-indigo-600">
                          ...
                          <div className="relative z-10 flex items-center gap-1 overflow-x-auto overscroll-contain py-1 scrollbar-none border-b border-slate-200/50">
                            {[
                              ...
                              { id: 'featureMenu', label: 'Feature Menu' }
                            ].map(t => (
                              <button
                                key={t.id}
            ...
            {/* CHAT INPUT BAR */}
            <form onSubmit={handleSendMessage} className="relative z-10 p-3 bg-white/80 backdrop-blur-xl border-t border-white/80 flex items-center gap-2 shrink-0 shadow-lg">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search card name (e.g., CDR, SDR)..."
                  className="w-full bg-slate-100/80 focus:bg-white/95 text-slate-900 placeholder:text-slate-400 text-xs font-semibold rounded-xl pl-9 pr-3 py-2.5 border border-white/90 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none backdrop-blur-md transition-all shadow-inner"
                />
              </div>
              <button
                type="submit"
