const fs = require('fs');
const pages = ['EBI', 'LPGRI', 'PIM', 'PANI', 'VII', 'RCI', 'BCI', 'CCI'];
const titles = {
  EBI: 'Election Bureau Intelligence',
  LPGRI: 'LPG Record Intelligence',
  PIM: 'Passport Intelligence Module',
  PANI: 'PAN Card Intelligence',
  VII: 'Voter ID Intelligence',
  RCI: 'Ration Card Intelligence',
  BCI: 'Birth Certificate Intelligence',
  CCI: 'Caste Certificate Intelligence'
};

const template = (id, title) => `import React from 'react';
...
export default function ${id}Page({ onBack }) {
  React.useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
...
pages.forEach(p => {
  fs.writeFileSync(`src/page/${p}.jsx`, template(p, titles[p]));
});
console.log('Created 8 pages');
